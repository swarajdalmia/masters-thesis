...
def train_GAN(images, save_path, ckpt_save_name, batch_size, n_epochs, dsrmntr_l_rate,  gnrtr_l_rate , load = False, checkpoint_load = "Null"):
        ...
        # Specify a learning rate if need be. Default is 0.00001 
        rmsD = (tf.train.RMSPropOptimizer(learning_rate=dsrmntr_l_rate)
            .minimize(-d_loss, var_list=d_vars))
        rmsG = (tf.train.RMSPropOptimizer(learning_rate=gnrtr_l_rate)
        ...
        for i in range(batch_iterations):
                dLoss = dLossFake = dLossReal = realProb = fakeProb = []
                for j in range(d_over_g):
                    # update the discriminator 5 times for every time a generator is updated 
                    z_batch = np.random.uniform(-1, 1, size=[batch_size, z_dimensions])
                    real_image_batch = im_normal[np.random.randint(im_normal.shape[0], size = batch_size), :,:,:]
                    _,dLoss,dLossFake, dLossReal, realProb, fakeProb, _ = sess.run([rmsD, d_loss, d_loss_fake, d_loss_real, Dx, Dg, clip_d],feed_dict={z_placeholder:z_batch,x_placeholder:real_image_batch}) #Update the discriminator
                z_batch = np.random.uniform(-1, 1, size=[batch_size, z_dimensions])
                _,gLoss = sess.run([rmsG,g_loss],feed_dict={z_placeholder:z_batch})
                ...
                if (i % int(im_normal.shape[0]/batch_size) == 0 and i > 1):
                        print("Fnished Epoch ", int(i/int(im_normal.shape[0]/batch_size)))
...
if __name__ == '__main__':
        ...
        # load = False
        image_size = (128,128)
        save_path = base_path + '/Results/wgan/' + image_tp
        ...
        train_GAN(images, save_path+str(1), ckpt_save_name, batch_size = 40, n_epochs= 150, dsrmntr_l_rate= 0.0001 , gnrtr_l_rate=0.0001)
        # train_GAN(images, save_path+str(2), ckpt_save_name, batch_size = 40, n_epochs= 150, dsrmntr_l_rate= 0.0001, gnrtr_l_rate=0.0001)
        # train_GAN(images, save_path+str(3), ckpt_save_name, batch_size = 40, n_epochs= 150, dsrmntr_l_rate= 0.0001, gnrtr_l_rate=0.0001)
