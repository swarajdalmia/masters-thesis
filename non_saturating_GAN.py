...
def train_GAN(images, save_path, ckpt_save_name, batch_size, n_epochs, dsrmntr_l_rate,  gnrtr_l_rate , load = False, checkpoint_load = "Null"):
        ...
        x_placeholder = gaussian_noise_layer(x_placeholder, .1)
        ...
        # Adding noise to images generated from the generator 
        Gnoise = gaussian_noise_layer(Gz, .1)
        Dg = discriminator(Gnoise, color, reuse=True) #Dg will hold discriminator prediction probabilities for generated images

        # un saturating GAN loss function,  >> minimize log(1 – D(G(z)))
        g_loss = tf.reduce_mean(tf.math.log(tf.ones_like(Dg) - Dg))
...
if __name__ == '__main__':
        ...
        # load = False
        image_size = (128,128)
        save_path = base_path + '/Results/rms_loss_gan/' + image_tp
        ...
        train_GAN(images, save_path+str(1), ckpt_save_name, batch_size = 40, n_epochs= 200, dsrmntr_l_rate= 0.001, gnrtr_l_rate=0.005)
