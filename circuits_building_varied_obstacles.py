...
def main():
    ...
    base = '/Users/swarajdalmia/Desktop/NeuroMorphicComputing/Code/Data/circuitImages/'

    # 5*500 images will be stored 
    for j in range(4):
        prob_rand_global = 0.03*(j)
        prob_rand_local = 0.30 + 0.12*(j-1)
        i = 0
        while(i<=2500):
            num = j*2500 + i
            ...
            # circuit = build_circuit_background(j)
            # # display_circuit(circuit, 'Background Circuit')
            # # reached stpres whether the circuit reached the output wire 
            # save_circuit_simplified(circuit, base + 'usefulCircuits/test_obstacles/'+im_name)
            # i = i+1
            ...
            if (reached_top == False):
                circuit, reached_top = make_paths(c, [int(circuit_dim[0]/3 - 1),5], [int(circuit_dim[0]/3 - 1), circuit_dim[1]-6], 1, 8, upper_bound_steps, prob_rand_local, prob_rand_global)  # first input and first output 
            c = np.copy(circuit)
            ...
            if (reached_bottom == False):
                circuit, reached_bottom = make_paths(c, [int(2*circuit_dim[0]/3),5], [int(2*circuit_dim[0]/3), circuit_dim[1]-6] , 2, 9, upper_bound_steps, prob_rand_local, prob_rand_global) 
            # display_circuit(circuit, 'Finished Circuit')
            ...
            if(reached_top == True and reached_bottom == True):
                ...
                save_circuit(circuit, base + 'usefulCircuits/asIs_Legend/'+im_name)
                # remove the legends here
                save_circuit_simplified(circuit, base + 'usefulCircuits/asIs/'+im_name, True, True)
                # remove the noise but keep obstacles
                save_circuit_simplified(circuit, base + 'usefulCircuits/withObstacles_withoutNoise/'+im_name, True, False)
                # remove the obstacles but keep the noise 
                save_circuit_simplified(circuit, base + 'usefulCircuits/withoutObstacles_withNoise/'+im_name, False, True)
                a,b = path_lengths(circuit, 1,2)
                len_upper.append(a)
                len_lower.append(b)
                # greyscale with obstacles
                save_circuit_simplified(circuit, base + 'usefulCircuits/grey_withObstacles/'+im_name, True, False, True)
            else: 
                save_circuit(circuit, base + 'failedCircuits/'+im_name)
    ...
    np.savetxt(base + 'usefulCircuits/len_upper.txt', len_upper, fmt='%d')
    np.savetxt(base + 'usefulCircuits/len_lower.txt', len_lower, fmt='%d')
...
def build_circuit_background(j):    # higher the j, the more complex the obstacles
    ...
    # behind 
    row = int(circuit_dim[0]/3 - 1)       # 64/3 - 1 = 20
    col = 4                               # 4
    circuit[row, col] =  circuit[row-1:row+2, col+1] = 6
    col = circuit_dim[1] - 5              # 59
    circuit[row, col] =  circuit[row-1:row+2, col-1] = 7 

    row = int(2*circuit_dim[0]/3)         # 42
    col = 4 
    circuit[row, col] =  circuit[row-1:row+2, col+1] = 6
    col = circuit_dim[1] - 5
    circuit[row, col] =  circuit[row-1:row+2, col-1] = 7

    # add obstacles depending on the complexity between 11 and circuit_dim[1] - 12 depending on j 
    # higher the value of j, more complex the landscape for escapring obstacles 
    flag = True
    while(flag):
        # generate obstacle (rectangular obstacles) - generate n-rows, n-cols and top left corner
        nrows = random.randint(3,12)
        ncols = random.randint(3,12)
        pos_row = random.randint(4, 60-nrows+1)
        pos_col = random.randint(11, 52-ncols+1)
        # place the obstacle(dont check for interseaction because we are fine with complex obstacle structures.)
        circuit[pos_row:pos_row+nrows, pos_col:pos_col+ncols] = 0
        # evaluate what percentage of the box if filled and is its under the limit, keep adding obstacles. 
        percentage = percent_obstacle_filled(4,60,11,52, circuit)
        # print("precentage : ", percentage)
        # if(percentage > (0.2 + j*0.05)):   # vaires from 0.2 to 0.4

        if(percentage > (0.1 + j*0.05)):   # vaires from 0.1 to 0.3
            flag = False 
        
    # empty two entire rows withing the chunk to empty(so as to have a possibility of success atleast)
    row_empty = random.randint(4,59)
    circuit[row_empty:row_empty+2,11:53] = 5

    return circuit

#returns the precentage of the rectnagle bounded by (r1,c1) and (r2,c2) that is covered by obstacles
def percent_obstacle_filled(r1,r2,c1,c2, circuit):
    count = 0
    for i in range(r1,r2+1,1):
        for j in range(c1, c2+1, 1):
            if(circuit[i,j] == 0):
                count = count+1
    return count/((r2-r1)*(c2 - c1))
